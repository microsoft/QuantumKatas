		...
		for ((n, W, P, maxTotal, itemWeights, itemProfits, itemInstanceBounds, P_max) in ExampleSets()){
			...
			using (register = Qubit[Q]){
				...
				for (combo in combos){
					...
					mutable equivalent = true;
					for (i in 0..n-1){
						Fact(Length(xs[i]) == BinaryLength(itemInstanceBounds[i]), $"Unexpected result for combination {xsCombo} : The array at index {i} in the output jagged array has length {Length(xs[i])}, should have {BinaryLength(itemInstanceBounds[i])}.");
